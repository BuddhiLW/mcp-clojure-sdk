(ns jsonrpc4clj.server
  (:require
   ...
   [jsonrpc4clj.requests :as requests]
   [jsonrpc4clj.responses :as responses]
   [jsonrpc4clj.trace :as trace]
   [promesa.core :as p]
...
;; Avoid error: java.lang.IllegalArgumentException: Multiple methods in multimethod 'simple-dispatch' match dispatch value: class jsonrpc4clj.server.PendingRequest -> interface clojure.lang.IPersistentMap and interface clojure.lang.IDeref, and neither is preferred
...
(defn pending-request
  ...
  Most of the time, you should call `jsonrpc4clj.server/deref-or-cancel` on the
  ...
  `:jsonrpc4clj.server/cancelled`.

  Sends `$/cancelRequest` only once, though `jsonrpc4clj.server/deref-or-cancel` or
...
(defn ^:private format-error-code [description error-code]
  (let [{:keys [code message]} (errors/by-key error-code)]
...
(defn ^:private internal-error-response [resp req]
  (let [error-body (errors/internal-error (select-keys req [:id :method]))]
    (responses/error resp error-body)))

(defn ^:private cancellation-response [resp req]
  (let [message-details (select-keys req [:id :method])
        error-body (errors/body :request-cancelled
...
(defrecord ChanServer [input-ch
  ...
  (start [this context]
    ...
    (let [;; The language server sometimes needs to stop processing inbound
      ...
      (async/go
        ...
        ;; All that to say, it's possible for the jsonrpc4clj server to drop the
        ;; language server's final few responses.

        ;; It doesn't really matter though, because the users of jsonrpc4clj
        ;; typically don't call `shutdown` on the jsonrpc4clj server until they've
  ...
  (send-request [this method body]
    (let [id (swap! request-id* inc)
          now (.instant clock)
          req (requests/request id method body)
  ...
  (send-notification [this method body]
    (let [now (.instant clock)
          notif (requests/notification method body)]
